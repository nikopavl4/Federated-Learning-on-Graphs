...
#Train Dataset split to Clients - Unbalanced yes Overlaps no
startup = 0
Client_list = []
...
for i in range(args.clients):
    if i == 0:
        client_data = train_dataset[startup:divisions[i]+startup+20]
    elif i == 1:
        client_data = train_dataset[startup - 10:divisions[i]+startup+10]
    else:
        client_data = train_dataset[startup - 20:divisions[i]+startup]
...
client_global_loss_per_round= {k:[] for k in dict_keys}
...
for round_id in range(1,args.federated_rounds + 1):
    ...
    for MyClient in Client_list:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if float(loss2) < loss_min:
                loss_min = float(loss2)
            #Test on inter test set           
            ...
            if early_stopping_callback.early_stop:
                break
        #Keep all epochs best
        client_global_loss_per_round[MyClient.id].append(loss_min)
...
for i in range(args.clients):
    ...
    plt.figure(figsize=(10,5))
    plt.subplot(2,1,1)
    plt.plot(client_global_acc_per_round[i])
    plt.ylabel("Accuracy")
    plt.subplot(2,1,2)
    plt.plot(client_global_loss_per_round[i])
    plt.xlabel("Federated Round")
    plt.ylabel("Loss")
    plt.suptitle(f"Accuracy/Loss on global test set per Federated Round / Client {i}")
